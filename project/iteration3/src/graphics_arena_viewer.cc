...
GraphicsArenaViewer::GraphicsArenaViewer(
  const struct arena_params* const params)
    : csci3081::GraphicsApp(params->x_dim, params->y_dim, "player Simulation"),
      ...
      pause_btn_(nullptr),
      play_btn_(nullptr),
      Winner_Window(nullptr),
      Loser_Window(nullptr),
      last_dt(-1) {
...
void GraphicsArenaViewer::DrawSuperbot(NVGcontext *ctx,
  ...
  nvgRotate(ctx, superbot->heading_angle());
  ...
  nvgFillColor(ctx, nvgRGBA(static_cast<int>(superbot->get_color().r * 255),
                            static_cast<int>(superbot->get_color().g * 255),
                            static_cast<int>(superbot->get_color().b * 255),
                            255));
  ...
  nvgRotate(ctx, M_PI / 2.0);
  nvgFillColor(ctx, nvgRGBA(0, 0, 0, 255));
  nvgText(ctx, 0.0, 10.0, superbot->get_name().c_str(), NULL);
...
void GraphicsArenaViewer::DrawUsingNanoVG(NVGcontext *ctx) {
  ...
  for (size_t i = 0; i < superbots.size(); i++) {
    ...
    // DrawRobotSensors(ctx, superbots[i], 2.0, 90);
  }
