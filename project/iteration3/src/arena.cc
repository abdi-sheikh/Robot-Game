...
Arena::Arena(const struct arena_params* const params) :
  ...
  n_superbots_(params->n_superbots),
...
void Arena::UpdateEntitiesTimestep(void) {
  ...
  for (unsigned int i = 0; i < rob.size(); ++i) {
      CheckForEntityCollision(player_, robot, &ec, player_->collision_delta());
      if (ec.collided()) {
  //      printf("Player entity type %s\n",player_.get_entity_type() );
      ...
      /*
       * This checks for multiple different collisions.
       * We have the home base and robot collision that changes the robot to
       * a superbot. That is done by creating new params for superbot, and
       * updating it with the robots information. Once that is done you create
       * a new superbot with the robots information. Then you erase the robot
       * from the entities,mobile_entities_, and robot vectors. Then you add the
       * superbot to the entities, and mobile_entities_ vectors.
       *
       */
      ...
      if (ec.collided()) {
           std::cout << "BOOM \n";
          superbot_params sbparams;
          sbparams.radius = rob[i]->get_radius() * 2;
          sbparams.pos = rob[i] ->get_pos();
          sbparams.color = csci3081::Color(191, 63, 127, 1);
          Superbot * superbot_ = new Superbot(&sbparams, rob[i]);
          entities_.erase(std::remove(entities_.begin(), entities_.end(),
                                                  rob[i]), entities_.end());
          mobile_entities_.erase(std::remove(mobile_entities_.begin(),
                      mobile_entities_.end(), rob[i]), mobile_entities_.end());
          rob.erase(std::remove(rob.begin(), rob.end(), rob[i]), rob.end());
          n_robots_--;
      ...
      /*
       * This checks for a collision for two robots and
       * if a robot is frozen it will unfreeze by resetting the speed
       * and set its distressed sensor to 0;
       */
      for (unsigned int j = i + 1; j <rob.size(); j++) {
        CheckForEntityCollision(rob[i], rob[j], &ec,
          rob[i]->collision_delta());
        if (ec.collided()) {
          if (rob[i] ->get_speed() == 0 || rob[j] ->get_speed() == 0) {
            rob[j]->set_speed(5);
            rob[j]->set_distressed(0);
            rob[i]->set_speed(5);
            rob[i]->set_distressed(0);
            frozen--;
          }
        }
      }
    ...
    std::vector<class Superbot*> super(superbots());
    for (auto spb : super) {
      if (n_robots_ == 0) {
        lose = true;
      }
      /*
       * This checks for a collision between a superbot and player
       * if there is a collision call the froze function for player
       * that freezes the robot for about 6 seconds.
       *
       */
    for (unsigned int i = 0; i < super.size(); ++i) {
      CheckForEntityCollision(player_, spb, &ec, player_->collision_delta());
      if (ec.collided()) {
            player_->froze();
        }

        /*
         * This checks for a collision for robot and superbot and 
         * if a robot is frozen it will unfreeze by resetting the speed
         * and set its distressed sensor to 0;
         */
      CheckForEntityCollision(super[i], robot, &ec,
                              super[i]->collision_delta());
      if (ec.collided()) {
        if (robot ->get_speed() == 0) {
          robot->set_speed(5);
          robot->set_distressed(0);
          frozen--;
        }
    }
  }
